<template>
    <div>
        ...
        <div class="predictionsTable" v-show="predictions[0].Time!=0">
            ...
            <button @click="editLoto7UsersPredictions()" :disabled="isDisabled">Register</button>
            <button @click="deleteLoto7UsersPredictions()" :disabled="isDisabled">Delete</button>
            <table>
                <thead>
                    <tr>
                ...
                <tbody>
                    <tr v-for="(prediction, index) in predictions" :key="prediction.Time_Id">
                        ...
                        <td><input type="text" v-model="predictions[index].Number_7" :readonly="read[index]"></td>
...
export default {
    ...
    methods: {
        ...
        async editLoto7UsersPredictions() {
            let parameter = [];
            for (let i = 0; i < Object.keys(this.predictions).length; i++) {
                if (this.checkSingle[i]) {
                    ...
                    this.predictions[i].Number_7 = typeof this.predictions[i].Number_7 == 'string' ? this.predictions[i].Number_7 : String(this.predictions[i].Number_7);
            ...
            await axios.post("/editLoto7UsersPredictionsDetail", {body: {user_id: this.$store.getters.getUserId, predictions: parameter}});
        },
        async deleteLoto7UsersPredictions() {
            let parameter = [];
            for (let i = 0; i < Object.keys(this.predictions).length; i++) {
                if (this.checkSingle[i]) {
                    this.predictions[i].Time = typeof this.predictions[i].Time == 'string' ? this.predictions[i].Time : String(this.predictions[i].Time);
