...
func EditLoto6Result(body map[string]string) bool {
	param := body
	...
	before_edit_record := model.GetLoto6ResultDetail(time)
	...
	for _, number := range input_data {
		...
		if exist_only_input == false {
			updateStatisticsQuery[counter] = `UPDATE loto6_statistics SET count = (SELECT count + 1 FROM loto6_statistics WHERE number = $1), rate = (SELECT CAST(count + 1 AS float) FROM loto6_statistics WHERE number = $2) / (SELECT time FROM loto6_statistics WHERE number = $3) WHERE number = $4`
	...
	updateLoto6ResultsQuery := `UPDATE loto6_results SET number_1 = $1, number_2 = $2, number_3 = $3, number_4 = $4, number_5 = $5, number_6 = $6 WHERE time = $7`

	dbResult := model.UpdateLoto6Results(updateLoto6ResultsQuery, input_data, updateStatisticsQuery, params, time)
	return dbResult
}
