...
func GetLoto6UsersPredictions(body map[string]string) map[int]map[string]interface{} {
	...
	data[0]["Rate"] = float64(total) / ((float64(len(data)) - float64(beforeResultAnnouncementCount)) * float64(6)) * float64(100)
	data[0]["existAfterResultAnnouncement"] = true
	return data
}
