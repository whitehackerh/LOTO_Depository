...
func EditLoto7UsersPredictionsDetail(param *stash.EditLoto7PredictionsDetailStash) bool {
	...
	for i := 0; i < len(param.Body.Predictions); i++ {
		...
		input_data[i]["input_number_7"], _ = strconv.Atoi(param.Body.Predictions[i]["Number_7"])
	}

	dbResult := model.EditLoto7UsersPredictionsDetail(input_data, input_id)
	return dbResult
}
